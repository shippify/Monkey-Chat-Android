...
class DecryptTask(service: MonkeyKitSocketService): AsyncTask<EncryptedMsg, MOKMessage, Int>(){
    ...
    override fun onProgressUpdate(vararg values: MOKMessage?) {
        ...
        service?.delegateHandler?.processMessageFromHandler(CBTypes.onMessageReceived,
    ...
    companion object {
        fun decryptMessage(encrypted: EncryptedMsg): Boolean{
            try {
                ...
                if ((message.props?.get("encr")?.asString ?: "1").compareTo("1") == 0 && message.type != MessageTypes.MOKFile) {
                ...
                else if ((message.props?.has("encoding") ?: false) && message.type != MessageTypes.MOKFile) {
